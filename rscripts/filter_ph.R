...
# Usage: filter_ph.R path.to.ph path.to.anno path.to.tRNA path.to.snoRNA path.to.TFBS path.to.RM
args <- commandArgs(trailingOnly = TRUE)

path.to.ph <- '../python_scripts/folding_pretty_copy/out/folding/panhandles_preprocessed.tsv'
path.to.ph.bed <- '../python_scripts/folding_pretty_copy/out/folding/panhandles_preprocessed.bed'
n.ph.in.cluster. <- 15
...
n.ph.in.cluster. < args[3]
# path.to.anno <- args[4]
# path.to.tRNA <- args[5]
# path.to.snoRNA <- args[6]
# path.to.TFBS <- args[7]
# path.to.RM <- args[8]
...
FilterUTR <- function(ph, path.to.anno){
...
IntersectExons <- function(path.to.ph.bed12, path.to.anno){
  x <- paste('tail -n +6 ', path.to.anno, 
             ' | awk -F"\\t" \'$3 == "exon"\'| awk \'{printf ("%s\\t%s\\t%s\\n", $1, $4, $5)}\' | bedtools sort -i stdin | bedtools intersect -a ',
             path.to.ph.bed12, ' -b stdin -wa -wb', sep = '')
  return(read.delim(pipe(x), header = F)$V4)
}

FilterAlwaysIntronic <- function(ph, path.to.anno){
  ids.exons.left <- unique(IntersectExons(path.to.ph.bed12, path.to.anno))
  SwapHandles(path.to.ph.bed12)
  ids.exons.right <- unique(IntersectExons('swapt_handles.bed', path.to.anno))
...
ph <- FilterUTR(ph, path.to.anno)
ph <- FilterAlwaysIntronic(ph, path.to.anno)
ph <- Count_clusters(ph, path.to.ph.bed)

filtered <- ph[ph$always.intronic == T & 
                 ph$CDS.transcript == T & 
                 ph$tRNA == F & 
                 ph$sno.miRNA == F & 
                 ph$TFBS == F & 
