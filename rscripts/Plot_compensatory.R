...
comp.shuffled <- '../python_scripts/compensatory_copy/out/filtered/N_compensatory_shuffled.txt'
pairs.shuffled <- '../python_scripts/compensatory_copy/out/filtered/N_pairs_with_SNP_shuffled.txt'
comp <- 8
pairs <- 41427
title <- 'filtered_ds, 1000 iterations, bins25'
...
png(paste0('Distribution_of_compensatory_shuffled_in_', title, '.png'), res = 300, width = 9, height = 7, units = 'in')
...
text(real.ratio, height * 0.8, cex = 1.3, pos = 4, "Observed", col = 'red')
text(width.min, height * 0.8, cex = 1.3, pos = 4, "Expected", col = 'black')
dev.off()
