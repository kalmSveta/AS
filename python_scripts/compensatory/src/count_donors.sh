...
for file in $1*out.txt; do 
	#gunzip $file 
	#file_name=${file//txt.gz/txt}
	file_name=$file
	echo $file_name 
	cut -f 16- $file_name | awk '{print $0,"\t",gsub(/1/,"")}' | awk '{print $0,"\t",gsub(/1[|]1/,"")}'| awk '{print $(NF-1) - $NF}' | paste $file_name - > "$file_name".counts ; 
	#gzip $file_name
done
#takes info only about mutations and number of donors
for file in $1*counts; do 
	echo $file
	awk 'BEGIN{OFS="\t"} {print $7,$8,$9,$10,$11,$(NF)}' < $file > $file.2 ; 
done
rm $1/*counts
