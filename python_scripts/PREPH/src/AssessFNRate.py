...
def Find_Random_panhandles(path_to_intervals, energy_threshold, handle_length_threshold, panhandle_length_threshold, k,
                    genome_file, threads, need_suboptimal, kmers_stacking_matrix, N_seeds):
    ...
    if not ('sequences' in list(df.columns.values)):
        ...
        genome = Fasdf['start_interval'] = df['start_interval'].astype(int)(genome_file)
    ...
    for seed in range(1,N_seeds + 1):  
        ...
        ##shuffle
        df_new = df.copy() 
        df_new = Shuffle(df_new, 'sequences', seed)
...
def main(argv):
    ...
    N_seeds = 10
    try:
        opts, args = getopt.getopt(argv, "h:i:g:k:p:a:t:e:u:d:N:",
                                   ["help=", "intervals=", "genome=", "k=", "panh_len_max", "handle_len_min", "threads",
                                    "energy_max", "need_subopt", "gt_threshold"])
    except getopt.GetoptError:
        print(
            'AssessFNRate.py -i <intervals_df> -g <genome.fa> -k <kmer_lentgh> -p <panhandle_len_max> ' +
            '-a <handle_len_min> -t <threads> -e <energy_max> -u <need_suboptimal> -d <gt_threshold> -N <N_seeds>')
        sys.exit(2)
    for opt, arg in opts:
        if opt in ('-h', '--help'):
            print(
